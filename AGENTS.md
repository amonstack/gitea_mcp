...
- `make test` runs unit tests via `vitest run`; a runtime smoke run of the built `dist/` (e.g. `node dist/cli.js skills install --dir <tmp>` and constructing the server from `dist/server.js`) covers the runtime/type-decoupling and module-resolution classes that neither emit nor `--noEmit` can catch.
